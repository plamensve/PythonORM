...
def show_all_authors_with_their_books() -> str:
    ...
    for author in authors:
        ...
        author_with_books.append(f"{author.name} has written - {titles}!")
...
def calculate_average_rating_for_product_by_name(product_name: str):
    ...
    review = product.reviewss.all()

    result = []

    for r in review:
        result.append(r.rating)

    avg_rating = sum(result) / len(result)
...
def get_products_with_no_reviews():
    return Product.objects.filter(reviewss__isnull=True).order_by('-name')
...
def get_drivers_with_expired_licenses(due_date: date):
    ...
    for d in driver:
        license_expire = d.license.issue_date + timedelta(days=365)
