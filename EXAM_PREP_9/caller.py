...
def get_profiles(search_string=None):
    ...
    profile = Profile.objects.annotate(num_of_orders=Count('order')).filter(query).order_by('full_name')
    if not profile:
        return ''
...
def get_loyal_profiles():
    ...
    result = []
    for p in profiles:
        result.append(f"Profile: {p.full_name}, orders: {p.num_of_orders}")
    return '\n'.join(result)


def get_last_sold_products():
    try:
        order = Order.objects.all().prefetch_related('products').latest('creation_date')
